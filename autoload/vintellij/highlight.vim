...
function! vintellij#highlight#SetHighlights(buffer) abort
    let l:insight_map = vintellij#bvar#get(a:buffer, 'insight_map')
    if empty(l:insight_map)
        let l:insight_map = {}
    endif
    let l:highlight_ids_to_remove = vintellij#bvar#get(a:buffer, 'highlight_ids')
    ...
    call vintellij#bvar#set(a:buffer, 'highlight_ids', l:highlight_ids)
endfunction
