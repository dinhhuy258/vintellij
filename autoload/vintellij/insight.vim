function! vintellij#insight#Jump(direction, ...) abort
  try
    let s:curLine = getpos('.')[1]
    let s:curCol = getpos('.')[2]
    let s:insightMap = vintellij#bvar#get(bufnr(''), 'insight_map')
...
function! s:FindNearestCol(direction) abort
...
function! s:FindNearestLine(direction) abort
...
function! s:ResetNearestLocation(direction)
  if a:direction == 'before'
    let s:nearestLine = 0
    let s:nearestCol = 0
  else
...
function! s:SetStartCountingCol(direction)
  if a:direction == "before"
    let s:curCol = 9999
  else
    let s:curCol = 0
...
function! s:SetNewStartLine(direction)
  if a:direction == "before"
